...
def do_everything(**kwargs):
    ...
    for metal_dir in np.array(metal_dirs)[metals]:
        ...
        agb_tracks = get_afile(os.path.join(working_dir, metal_dir),
                               track_identifier)
        agb_tracks.sort()
        ax = plt.axes()
        kwargs['ax'] = ax
        ...
        out.write(('# age(yr) logL logTe m_act mcore c/o period ip ',
                   'Mdot(Msun/yr) logTe X Y CNO dlogTe/dlogL \n'))
        ...
        for agb_track in agb_tracks:
            ...
            if diagnostic_dir is not None:
                diag_plots(track, logl, logt, age, slopes, Qs, addpt, rising,
                           fits, **kwargs)
